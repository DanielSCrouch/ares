...
public class JavaFF
{
  ...
  for (int depthBound = 5; depthBound < 100; ++depthBound)
  {
    ...
    goalState = HCS.search(); // Try and find a plan using HC

    if (goalState != null) // if path found break increasing depth search
    {
      infoOutput.println("Found goal, ending search");
      return goalState;
    }
  }
  ...
  {
    infoOutput.println("HC failed, using EHC, with all actions");
